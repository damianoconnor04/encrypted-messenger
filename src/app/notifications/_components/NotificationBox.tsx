...
const NotificationBox: React.FC<NotificationBoxProps> = ({ name, message, time, wasOpened, type, id }) => {
  ...
  return (
    <>
      {!removed && (type !== 'flagged') && (
        <motion.a initial={{ paddingRight: '0.75rem', translateY: '-100%' }} animate={{ translateY: '0%' }} transition={{ ease: "linear", duration: 0.3 }} onMouseEnter={() => setHover(true)} onMouseLeave={() => setHover(false)} whileHover={!rightHover && !midHover && !leftHover ? { paddingRight: '3.25rem' } : { paddingRight: '8.5rem' }} href="#" className={`mt-2 xl:mt-1 border border-transparent hover:dark:border-d-border hover:border-l-border hover:dark:bg-d-softbg hover:bg-l-softbg transition-colors  relative overflow-hidden flex items-center justify-center max-w-full gap-2 p-3 py-4 rounded-xl ${read || type === 'read' ? '' : ''} ${wasOpened && type === 'unread' ? 'hidden' : !wasOpened && type === 'read' && 'hidden'}`}>

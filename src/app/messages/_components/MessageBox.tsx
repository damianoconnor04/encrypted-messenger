...
const MessageBox:React.FC<MessageBoxProps> = ({ name, message, time, isRead }) => {
  ...
  return (
    <Link href="#" className='z-10 t-bg-hover flex items-center max-h-full gap-2 p-2 rounded-lg transition-colors'>
      <UserImg letter={name.charAt(0)} color={getRandomColor()} size='lg' />
      <div className='t-text min-w-0 flex flex-col justify-between w-full'> {/* min-w-0 is magic fix for truncate */}
        ...
        <div className='flex items-center justify-between'>
          <p className={`text-sm truncate ${isRead && 'font-light t-text-light'}`}>{message}</p>

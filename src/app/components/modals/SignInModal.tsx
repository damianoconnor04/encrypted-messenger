...
const SignInModal = ({ isOpen, onClose }: { isOpen?: boolean; onClose: () => void }) => {
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title='Sign in to Datadog' maxW="md">
      {/* <h3 className='text-3xl dark:text-white text-black font-bold md:hidden px-6'>Sign in to Datadog</h3> */}
      <form className='p-6 flex flex-col justify-between md:justify-normal h-full'>
        <div>
          <div className='flex max-[449px]:flex-col md:flex-col gap-2'>
            <Button type='button' variant='sso'><span className='text-lg'><FcGoogle /></span>Sign in with Google</Button>
            <Button type='button' variant='sso'><span className='text-lg'><FaApple /></span>Sign in with Apple</Button>
          </div>
          ...
          <div className='mt-4'>
            ...
            <Button type='button' variant='tertiary'>Forgot password?</Button>
          </div>
        ...
        <div className='block md:hidden'><Button type='submit' variant='primary' onClick={() => { }}>Continue</Button></div>
      </form>
    </Modal>
  )
}
